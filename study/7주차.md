...
**클래스의 전체 이름은 "패키지명 + 클래스명"**이며 계층 구조의 패키지를 점(.)을 사용해 구분
```
├── src                           # src 하위부터 패키지 경로 #
...
```
또한, **패키지가 있는 클래스를 실행하려면 생성한 바이트코드가 있는 폴더가 아닌 패키지가 시작하는 폴더**에서 ```java``` 명령어로 실행 (패키지는 클래스명의 일부이기 때문에)
...
public class Ex {
  public static void main(String[] args) {
    // 컴파일러는 dami.hackerrank.medium의 A 클래스와 dami.leetcode.easy 패키지의 A 클래스 중 어떤 클래스를 로딩해야하는지 알지 못하기 때문에 컴파일 에러 발생
...
## -classpath 옵션
자바 컴파일 명령 시 클래스패스를 지정하는 옵션. ```-cp```로 단축 사용 가능
```
...
(현재 경로 : /)
```
javac -classpath . /Users/mongzza/java/classes/MyClass.java
```
다른 경로에 추가할 클래스 파일이 더 있다면 콜론(:) 사용해 여러개 설정
```
javac -cp .:/Users/mongzza/java/classes/util:/Users/valid.jar /Users/mongzza/java/classes/MyClass.java
```
